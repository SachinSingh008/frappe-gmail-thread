...
from frappe_gmail_thread.frappe_gmail_thread.doctype.gmail_thread.gmail_thread import sync_labels
...
class GmailAccount(Document):
    ...
    def on_update(self):
        ...
        if self.has_value_changed("labels") and not self.has_value_changed("last_historyid"):
            if self.gmail_enabled and self.refresh_token and not self.has_value_changed("refresh_token"):
                has_labels = False
